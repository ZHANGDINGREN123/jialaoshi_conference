...
def get_train_data(scale = 0.8):
    df = pd.read_csv("./GPS_Data/1_maxmin/1_maxmin_1and2.csv", names=['1', '2', '3', '4',
                                        '5', 'kongbai', '7', '8', '9', '10', '11'])
    df_matrix = df.as_matrix()
    df_matrix_f32 = np.float32(df_matrix)
    x_train_input1 = df_matrix_f32[0:74300, 0:5]
    a1 = np.ones(74300)[np.newaxis]
    b1 = np.zeros(74300)[np.newaxis]
    c1 = np.vstack((a1, b1))
    x_train_label1 = c1.T
    x_train_input2 = df_matrix_f32[0:74300:, 6:]
    a2 = np.ones(74300)[np.newaxis]
    b2 = np.zeros(74300)[np.newaxis]
    c2 = np.vstack((b2, a2))
    x_train_label2 = c2.T
...
def get_test_data(scale = 0.8):
    ...
    df_matrix = df.as_matrix()
    df_matrix_f32 = np.float32(df_matrix)
    x_test_input1 = df_matrix_f32[74300:, 0:5]
    a1 = np.ones(74300)[np.newaxis]
    b1 = np.zeros(74300)[np.newaxis]
    c1 = np.vstack((a1, b1))
    x_test_label1 = c1.T
    x_test_input2 = df_matrix_f32[74300:, 6:]
    a2 = np.ones(74300)[np.newaxis]
    b2 = np.zeros(74300)[np.newaxis]
    c2 = np.vstack((b2, a2))
    x_test_label2 = c2.T
