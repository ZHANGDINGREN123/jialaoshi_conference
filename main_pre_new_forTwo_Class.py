...
def get_train_data(scale = 0.8):
    df = pd.read_csv("./GPS_Data/1_zeromean/1_zeromean_1and2.csv", names=['1', '2', '3', '4',
    ...
    c1 = np.vstack((a1, b1))
    x_train_label1 = c1.T
    x_train_input2 = df_matrix_f32[0:95744:, 6:]
...
def get_test_data(scale = 0.8):
    ...
    c1 = np.vstack((a1, b1))
    x_test_label1 = c1.T
    x_test_input2 = df_matrix_f32[95744:, 6:]
    a2 = np.ones(52856)[np.newaxis]
    b2 = np.zeros(52856)[np.newaxis]
    c2 = np.vstack((b2, a2))
    x_test_label2 = c2.T
